  ...
  },
  {
    ...
    input_schema: {
      ...
      properties: {
        ...
        progressDelta: { type: 'number', description: 'For milestone_delta only: fraction of THIS GOAL completed by this activity, −1..1. For countable goals ("ship 2 products", "read 5 books"), use 1/N per item: 1 of 2 = 0.5, 1 of 3 ≈ 0.33. For non-countable milestones: minor ≈ 0.05, meaningful ≈ 0.2, substantial ≈ 0.5. Use 1.0 only when the entire goal is 100% done. Omit for other kinds.' },
...
- LOGGING: The moment the user mentions doing ANYTHING — workout, run, reading, work session, habit, saving money, social event, anything at all — call log_progress immediately. Do NOT ask for more info. Do NOT wait. Just infer the right vector from the list above (use the exact [id] shown), pick the goal's kind (shown as kind=... next to each goal), and call the tool. For consistency goals, use kind=consistency_occurrence and occurredCount=1. For metric goals, use kind=metric_value with the value they mentioned. For milestone goals, use kind=milestone_delta — you MUST include goalId (milestone inputs without a goalId are split across all goals in the vector, giving the wrong number). progressDelta is the fraction of THE GOAL (not the sub-task) completed: for countable goals use 1/N per item done (e.g. 1 of 2 products shipped = 0.5, not 1.0), for non-countable milestones use your best estimate of how much of the goal this moves (minor ≈ 0.05, meaningful ≈ 0.2, substantial ≈ 0.5). Use progressDelta=1.0 ONLY when the entire goal is finished.
